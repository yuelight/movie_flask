...
def movie_edit(id=None):
    ...
    if form.validate_on_submit():
        ...
        if movie.title == data['title'] and movie_count == 1:
            flash('名称已经存在！', 'err')
            return render_template('admin/movie_add.html', form = form, movie = movie)
        if not os.path.exists(app.config['UP_DIR']):
            os.makedirs(app.config['UP_DIR'])
            os.chmod(app.config['UP_DIR'], 'rw')
...
def preview_edit(id=None):
    ...
    if form.validate_on_submit():
        ...
        if preview.title == data['title'] and preview_count == 1:
            flash('名称已经存在！', 'err')
            return render_template('admin/preview_add.html', form = form, preview = preview)
        if form.logo.data.filename != '':
            file_logo = secure_filename(form.logo.data.filename)
